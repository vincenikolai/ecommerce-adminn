...
export default function POManagerPage() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "created_at" | "stock" | "price" | "supplier_shop" | "date">("name"); // Added new sort options
  ...
  return (
    <div className="p-6">
      ...
      <Button onClick={() => setShowCreateSupplierManagementModal(true)} className="mb-4">Add New Supplier Management Item</Button>

      <div className="flex space-x-4 mb-4">
        <div>
          <Label htmlFor="sortBy">Sort By</Label>
          <Select onValueChange={(value: "name" | "created_at" | "stock" | "price" | "supplier_shop" | "date") => setSortBy(value)} value={sortBy}>
            ...
            <SelectContent>
              ...
              <SelectItem value="supplier_shop">Supplier Shop</SelectItem> {/* New sort option */}
              <SelectItem value="date">Date</SelectItem> {/* New sort option */}
            </SelectContent>
          </Select>
        </div>
        <div>
          ...
          <table className="min-w-full bg-white border border-gray-200">
            ...
            <tbody>
              {supplierManagementItems.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50"><td className="py-2 px-4 border-b">{item.name}</td><td className="py-2 px-4 border-b">{item.description}</td><td className="py-2 px-4 border-b">{item.price}</td><td className="py-2 px-4 border-b">{item.stock}</td><td className="py-2 px-4 border-b">{item.supplier_shop}</td><td className="py-2 px-4 border-b">{item.date}</td><td className="py-2 px-4 border-b space-x-2"><Button onClick={() => handleEditSupplierManagementItem(item)}>Edit</Button><Button onClick={() => handleDeleteSupplierManagementItem(item.id)} variant="destructive">Delete</Button></td></tr>
              ))}
            </tbody>
          </table>
        </div>
