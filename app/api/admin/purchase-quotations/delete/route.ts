...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    // Due to ON DELETE CASCADE on PurchaseQuotationItem, deleting the main order will also delete its associated items.
    ...
    if (deleteError) {
      console.error("API Route - Error deleting customer order:", deleteError);
      return NextResponse.json({ error: deleteError.message }, { status: 500 });
    }

    return NextResponse.json({ message: "Customer order deleted successfully", id });
  } catch (error: unknown) {
    console.error("API Route - Unexpected error in customer order deletion API:", error);
