...
export async function POST(req: Request) {
  try {
    ...
    if (profile?.role !== FINANCE_MANAGER_ROLE && profile?.role !== "admin") {
        return new NextResponse("Unauthorized: You do not have permission to create purchase invoices.", { status: 403 });
    }
    ...
    // Materials can be optional based on your schema design (JSONB can be null)
    // if (!materials) {
    ...
    console.log('[PURCHASE_INVOICE_POST]', error);
