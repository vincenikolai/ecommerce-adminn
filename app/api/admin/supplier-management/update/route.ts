...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { id, company_name, contact_person, email, phone, address, notes } = await req.json();

    if (!id || !company_name) {
      return NextResponse.json({ error: "Supplier ID and company name are required." }, { status: 400 });
    }

    const { data, error: updateError } = await localAdminSupabase
      .from('supplier_management_items')
      .update({ company_name, contact_person, email, phone, address, notes })
      .eq('id', id)
      .select();
    ...
    return NextResponse.json({ message: "Supplier updated successfully", supplier: data[0] });
  } catch (error: unknown) {
