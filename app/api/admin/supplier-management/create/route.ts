...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { company_name, contact_person, email, phone, address, notes } = await req.json();
    ...
    const { data, error: insertError } = await localAdminSupabase
      ...
      .insert([{ company_name, contact_person, email, phone, address, notes }])
      .select();
    ...
    return NextResponse.json({ message: "Supplier created successfully", supplier: data[0] });
  } catch (error: unknown) {
