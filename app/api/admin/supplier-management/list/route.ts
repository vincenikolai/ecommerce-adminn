...
export async function GET(req: Request) {
  ...
  try {
    ...
    console.log("API Route - Fetching suppliers from Supabase...");

    let { data: suppliers, error: fetchError } = await localAdminSupabase
      .from('supplier_management_items')
      .select('*');
    console.log("API Route - Suppliers Data:", suppliers);
    if (fetchError) {
      console.error("API Route - Error fetching suppliers:", fetchError);
    ...
    suppliers.sort((a, b) => {
      ...
      if (sortBy === "company_name") {
        valA = a.company_name || a.supplier_shop; // Backward compatibility
        valB = b.company_name || b.supplier_shop;
