...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!id || !name || !price || stock === undefined) {
      return NextResponse.json({ error: "Supplier management item ID, name, price, and stock are required." }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Supplier management item updated successfully", product: data[0] });
  } catch (error: unknown) {
