...
export async function middleware(req: NextRequest) {
  ...
  // Allow access to sign-in page if explicitly signing out (query param signout=true)
  const isSigningOut = req.nextUrl.searchParams.get('signout') === 'true';
  
  if (session && (isSignInPage || req.nextUrl.pathname.startsWith('/sign-up'))) {
    // If user is signing out, allow them to access sign-in page
    if (isSigningOut) {
      // Sign out the user and allow access to sign-in page
      await supabase.auth.signOut();
      return res;
    }
    // Otherwise, redirect authenticated users trying to access sign-in/sign-up pages
