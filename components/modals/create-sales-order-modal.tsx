...
interface CreateSalesOrderModalProps {
  ...
  quotationsToConvert: { id: string; supplier?: { name: string; supplier_shop: string }; materials: { id: string; rawMaterialId: string; quantity: number }[] }[]; // Changed to rawMaterialId
                ...
                return ( 
                  <div key={quotation.id} className="mb-4 p-2 border rounded-md bg-gray-50">
                    <p className="font-medium">
                      Quotation ID: {quotation.id} {quotation.supplier?.supplier_shop ? `(Supplier: ${quotation.supplier.supplier_shop})` : ""}
                    </p>
